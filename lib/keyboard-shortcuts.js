...
define(function (require) {
  ...
  function keyboardShortcuts () {
      ...
      for (var key in this.attr.shortcuts) {
        if (this.attr.shortcuts.hasOwnProperty(key)) {
          ...
          shortcut.forEach(function(shortcut) {
            ...
            var callback = this.shortcutEventHandlerFactory(shortcut);
    ...
    this.shortcutEventHandlerFactory = function(shortcut) {
      ...
      if (shortcut.throttle) {
        throttledEventHandler = this.throttle(eventHandler, 100);
        return throttledEventHandler;
      } else {
        debouncedEventHandler = this.debounce(eventHandler, 200);
